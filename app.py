...
try:
    from streamlit_sortables import sort_items, sorted_items
    _HAS_SORTABLES = True
except ImportError:
    pass
...
with st.sidebar:
    ...
    st.session_state.rename_enabled = st.checkbox("启用批量重命名", value=st.session_state.rename_enabled)
    if st.session_state.rename_enabled:
        tpl = st.text_input(
            "命名模板",
            value=st.session_state.rename_template,
            help="{stem}原名 {index}序号 {date}拍摄日期 {camera}相机 {gps_short}位置简写",
        )
    ...
    if not st.session_state.image_paths:
        ...
        with st.expander(f"📸 图片缩略图 ({len(st.session_state.image_paths)} 张)", expanded=True):
            ...
            for r in range(rows):
                ...
                for c in range(cols_per_row):
                    ...
                    with cols[c]:
                        try:
                            thumb = generate_thumbnail(st.session_state.image_paths[idx], (150, 150))
        ...
        if 0 <= selected_idx < len(st.session_state.image_paths):
            ...
            with col_prev_ctrls:
                ...
                with prev_col:
                    if st.button("⬅️ 上一张", use_container_width=True, disabled=selected_idx == 0):
                        st.session_state.selected_image_idx -= 1
                        _invalidate_preview_cache()
                        st.rerun()
                with next_col:
                    if st.button("下一张 ➡️", use_container_width=True, disabled=selected_idx >= len(st.session_state.image_paths) - 1):
                        st.session_state.selected_image_idx += 1
                        _invalidate_preview_cache()
                        st.rerun()
            ...
            if original is not None and st.session_state.pipeline is not None:
                ...
                if st.session_state.pipeline.steps:
                    cache_key = (img_path, json.dumps(st.session_state.pipeline.to_dict(), sort_keys=True))
                ...
                if processed is not None and st.session_state.pipeline.steps:
                    with st.expander("🔍 每一步效果预览", expanded=False):
                        cache_key = (img_path, json.dumps(st.session_state.pipeline.to_dict(), sort_keys=True))
                        _, step_results = st.session_state.preview_cache.get(cache_key, (None, []))
                        enabled_steps = [s for s in step_results if s.get("enabled") and "image" in s]
                        if enabled_steps:
                            step_cols = st.columns(min(5, len(enabled_steps)))
                            for i, (sc, sr) in enumerate(zip(step_cols, enabled_steps)):
                                with sc:
                                    info_step = STEP_REGISTRY.get(sr["step_type"], {})
...
with col_pipeline:
    ...
    if not st.session_state.pipeline.steps:
        ...
        if _HAS_SORTABLES and n_steps > 1:
            ...
            for i, step in enumerate(st.session_state.pipeline.steps):
                si = STEP_REGISTRY.get(step.step_type, {"name": step.step_type, "category": "?"})
                drag_items.append(f"{i+1}. [{si.get('category','')}] {si['name']}")
            new_order = sorted_items(drag_items, multi_containers=False, key=f"sortable_steps")
            if new_order and len(new_order) == n_steps:
                old_keys = [f"{i+1}. [{STEP_REGISTRY.get(st.session_state.pipeline.steps[i].step_type, {'name':'','category':'?'}).get('category','')}] {STEP_REGISTRY.get(st.session_state.pipeline.steps[i].step_type, {'name':st.session_state.pipeline.steps[i].step_type}).get('name','')}" for i in range(n_steps)]
                if new_order != old_keys:
                    idx_map = {old_keys[j]: j for j in range(n_steps)}
                    reordered = [st.session_state.pipeline.steps[idx_map[item]] for item in new_order if item in idx_map]
            ...
            st.markdown("**🔀 拖拽调整顺序**（输入位置编号）")
            cols_drag = st.columns(min(n_steps, 8))
            drag_positions = []
            for i in range(min(n_steps, 8)):
                with cols_drag[i]:
                    si = STEP_REGISTRY.get(st.session_state.pipeline.steps[i].step_type, {"name": st.session_state.pipeline.steps[i].step_type, "category": "?"})
                    pos = st.number_input(
                        f"{i+1}.{si['name'][:8]}",
            ...
            if len(drag_positions) == n_steps and n_steps > 1:
                ...
                reordered = [st.session_state.pipeline.steps[new_order[i]] for i in range(n_steps)]
                old_serialized = json.dumps([s.to_dict() for s in st.session_state.pipeline.steps], sort_keys=True)
                new_serialized = json.dumps([s.to_dict() for s in reordered], sort_keys=True)
        ...
        for i, step in enumerate(st.session_state.pipeline.steps):
            ...
            with col_up:
                if st.button("⬆", key=f"up_{i}", disabled=(i == 0), help="上移"):
                    ...
                    st.rerun()
            with col_dn:
                if st.button("⬇", key=f"dn_{i}", disabled=(i == len(st.session_state.pipeline.steps) - 1), help="下移"):
                    s = st.session_state.pipeline.steps.pop(i)
                    st.session_state.pipeline.steps.insert(i + 1, s)
                    _invalidate_preview_cache()
                    st.rerun()
            ...
            if step.enabled:
                ...
                with st.expander("⚙️ 参数配置", expanded=is_expanded):
    ...
    can_process = has_images and has_steps and not is_processing and not is_done
...
def _run_batch_processing():
    ...
    success_col, fail_col, time_col, speed_col = metrics_cols
    metrics = {"success": 0, "fail": 0, "start": time.time()}
    ph = {
        "success": success_col.empty(),
        "fail": fail_col.empty(),
        "time": time_col.empty(),
        "speed": speed_col.empty(),
    }
    ...
    def progress_cb(done, total_count, latest_result):
        ...
        ph["success"].markdown(f"✅ 成功  \n**{metrics['success']}**")
        ph["fail"].markdown(f"❌ 失败  \n**{metrics['fail']}**")
        ph["time"].markdown(f"⏱️ 已用  \n**{elapsed:.1f}s**")
        ph["speed"].markdown(f"🚀 速度  \n**{speed:.1f}/s**")
...
def _show_results():
    ...
    st.success(f"成功 {success_count} / 失败 {fail_count}，总耗时 {total_elapsed:.1f} 秒")
    ...
    with col_zip:
        if output_dir and os.path.isdir(output_dir):
            zip_path = os.path.join(tempfile.gettempdir(), f"processed_{int(time.time())}.zip")
            ...
            with open(zip_path, "rb") as f:
                st.download_button(
                    ...
                    f,
                    file_name=f"processed_{int(time.time())}.zip",
    ...
    with col_stats:
        ...
        if times:
            ...
            - 总计: {sum(times):.1f} s (实际: {wall_time:.1f} s)
...
# 步骤参数渲染
# ============================================================
def _render_step_params(idx: int, step: PipelineStep, defaults: Dict[str, Any]):
    params = step.params
    t = step.step_type
    needs_rerun = False

    def _upd(key, value):
        nonlocal needs_rerun
        if params.get(key) != value:
            params[key] = value
            needs_rerun = True

    if t == "crop_smart":
        modes = [("rule_of_thirds", "三分法构图(推荐)"), ("face", "人脸居中"), ("subject", "主体检测居中"), ("fixed", "固定比例")]
        labels = [m[1] for m in modes]
        values = [m[0] for m in modes]
        cur = params.get("mode", "rule_of_thirds")
        sel = st.selectbox("裁剪模式", labels, index=values.index(cur) if cur in values else 0, key=f"p_crop_m_{idx}")
        _upd("mode", values[labels.index(sel)])
        ratios = [
        ...
        _upd("ratio", r_values[r_labels.index(r_sel)])

    elif t == "crop_fixed":
        r = st.number_input("宽高比 (宽/高)", 0.1, 10.0, float(params.get("ratio", 1.0)), 0.01, key=f"p_cf_r_{idx}")
        _upd("ratio", r)
        modes = ["center", "top", "bottom", "left", "right"]
        m_labels = ["居中", "顶部", "底部", "左侧", "右侧"]
        sel = st.selectbox("锚点位置", m_labels, index=modes.index(params.get("mode", "center")), key=f"p_cf_m_{idx}")
        _upd("mode", modes[m_labels.index(sel)])

    elif t == "temp_tint":
        _upd("temperature", st.slider("色温 (冷蓝→暖黄)", -100, 100, int(params.get("temperature", 0)), key=f"p_tt_t_{idx}"))
        _upd("tint", st.slider("色调 (绿→品红)", -100, 100, int(params.get("tint", 0)), key=f"p_tt_i_{idx}"))

    elif t == "basic_adjust":
        for k in ["exposure", "contrast", "brightness", "highlights", "shadows", "whites", "blacks", "saturation", "vibrance"]:
            _upd(k, st.slider(k, -100, 100, int(params.get(k, 0)), key=f"p_ba_{k}_{idx}"))

    elif t == "hsl":
        _upd("hue", st.slider("色相偏移", -180, 180, int(params.get("hue", 0)), key=f"p_hsl_h_{idx}"))
        _upd("saturation", st.slider("饱和度", -100, 100, int(params.get("saturation", 0)), key=f"p_hsl_s_{idx}"))
        _upd("lightness", st.slider("明度", -100, 100, int(params.get("lightness", 0)), key=f"p_hsl_l_{idx}"))

    elif t == "color_balance":
        st.markdown("**阴影** (青-红 / 洋红-绿 / 黄-蓝)")
        c1, c2, c3 = st.columns(3)
        _upd("shadows_cr", c1.slider("S:青-红", -100, 100, int(params.get("shadows_cr", 0)), key=f"p_cb_scr_{idx}"))
        _upd("shadows_mg", c2.slider("S:洋-绿", -100, 100, int(params.get("shadows_mg", 0)), key=f"p_cb_smg_{idx}"))
        _upd("shadows_yb", c3.slider("S:黄-蓝", -100, 100, int(params.get("shadows_yb", 0)), key=f"p_cb_syb_{idx}"))
        st.markdown("**中间调**")
        c1, c2, c3 = st.columns(3)
        _upd("midtones_cr", c1.slider("M:青-红", -100, 100, int(params.get("midtones_cr", 0)), key=f"p_cb_mcr_{idx}"))
        _upd("midtones_mg", c2.slider("M:洋-绿", -100, 100, int(params.get("midtones_mg", 0)), key=f"p_cb_mmg_{idx}"))
        _upd("midtones_yb", c3.slider("M:黄-蓝", -100, 100, int(params.get("midtones_yb", 0)), key=f"p_cb_myb_{idx}"))
        st.markdown("**高光**")
        c1, c2, c3 = st.columns(3)
        _upd("highlights_cr", c1.slider("H:青-红", -100, 100, int(params.get("highlights_cr", 0)), key=f"p_cb_hcr_{idx}"))
        _upd("highlights_mg", c2.slider("H:洋-绿", -100, 100, int(params.get("highlights_mg", 0)), key=f"p_cb_hmg_{idx}"))
        _upd("highlights_yb", c3.slider("H:黄-蓝", -100, 100, int(params.get("highlights_yb", 0)), key=f"p_cb_hyb_{idx}"))
        ...
        if lut_items:
            ...
            sel = st.selectbox("滤镜预设", names, index=keys.index(cur) if cur in keys else 0, key=f"p_lut_{idx}")
            _upd("preset_name", keys[names.index(sel)])

    elif t == "curves":
        _upd("brightness", st.slider("亮度", -100, 100, int(params.get("brightness", 0)), key=f"p_cv_b_{idx}"))
        _upd("contrast", st.slider("对比度", -100, 100, int(params.get("contrast", 0)), key=f"p_cv_c_{idx}"))

    elif t == "resize_scale":
        _upd("scale", st.slider("缩放比例", 0.05, 4.0, float(params.get("scale", 1.0)), 0.05, format="%.2f×", key=f"p_rs_{idx}"))

    elif t == "resize_fit":
        _upd("width", st.number_input("最大宽度 (px)", 100, 20000, int(params.get("width", 1920)), key=f"p_rf_w_{idx}"))
        _upd("height", st.number_input("最大高度 (px)", 100, 20000, int(params.get("height", 1080)), key=f"p_rf_h_{idx}"))

    elif t == "resize_long_edge":
        _upd("max_long_edge", st.number_input("长边最大像素", 100, 50000, int(params.get("max_long_edge", 2048)), key=f"p_rle_{idx}"))

    elif t == "resize_short_edge":
        _upd("min_short_edge", st.number_input("短边最小像素", 100, 20000, int(params.get("min_short_edge", 1024)), key=f"p_rse_{idx}"))

    elif t == "resize_exact":
        _upd("width", st.number_input("目标宽度", 10, 50000, int(params.get("width", 800)), key=f"p_re_w_{idx}"))
        _upd("height", st.number_input("目标高度", 10, 50000, int(params.get("height", 600)), key=f"p_re_h_{idx}"))
        _upd("allow_upscale", st.checkbox("允许放大", params.get("allow_upscale", True), key=f"p_re_u_{idx}"))

    elif t == "smart_pad":
        _upd("width", st.number_input("目标宽度", 100, 20000, int(params.get("width", 1080)), key=f"p_sp_w_{idx}"))
        _upd("height", st.number_input("目标高度", 100, 20000, int(params.get("height", 1080)), key=f"p_sp_h_{idx}"))
        modes = ["auto", "color", "blur", "mirror", "repeat", "dominant"]
        labels = ["自动", "纯色", "模糊延伸", "镜像", "重复平铺", "主色调"]
        sel = st.selectbox("填充模式", labels, index=modes.index(params.get("mode", "auto")), key=f"p_sp_m_{idx}")
        _upd("mode", modes[labels.index(sel)])
        _upd("bg_color", st.color_picker("背景色(纯色模式)", params.get("bg_color", "#ffffff"), key=f"p_sp_c_{idx}"))

    elif t == "uniform_resolution":
        _upd("target_mp", st.slider("目标百万像素数 (MP)", 0.5, 100.0, float(params.get("target_mp", 24.0)), 0.5, format="%.1f MP", key=f"p_ur_{idx}"))

    elif t == "add_border":
        _upd("border_width", st.number_input("边框宽度 (px)", 1, 500, int(params.get("border_width", 20)), key=f"p_ab_w_{idx}"))
        _upd("color", st.color_picker("边框颜色", params.get("color", "#ffffff"), key=f"p_ab_c_{idx}"))

    elif t == "remove_bg":
        methods = ["auto", "ai", "traditional"]
        labels = ["自动(推荐)", "AI(rembg深度学习)", "传统(显著性+GrabCut)"]
        sel = st.selectbox("去背景方法", labels, index=methods.index(params.get("method", "auto")), key=f"p_rb_m_{idx}")
        _upd("method", methods[labels.index(sel)])
        rep = st.checkbox("背景替换为纯色(否则透明)", params.get("replace_with_color", False), key=f"p_rb_r_{idx}")
        _upd("replace_with_color", rep)
        if rep:
            _upd("bg_color", st.color_picker("背景色", params.get("bg_color", "#ffffff"), key=f"p_rb_c_{idx}"))

    elif t == "smart_sharpen":
        auto = st.checkbox("自动检测模糊并调整强度", params.get("auto_strength", True), key=f"p_ss_a_{idx}")
        _upd("auto_strength", auto)
        if not auto:
            _upd("strength", st.slider("锐化强度", 0.1, 3.0, float(params.get("strength", 1.0)), 0.1, key=f"p_ss_s_{idx}"))

    elif t == "unsharp_mask":
        _upd("amount", st.slider("数量/强度", 0.1, 5.0, float(params.get("amount", 1.0)), 0.1, key=f"p_um_a_{idx}"))
        _upd("radius", st.slider("半径", 0.1, 5.0, float(params.get("radius", 1.5)), 0.1, key=f"p_um_r_{idx}"))
        _upd("threshold", st.slider("阈值", 0, 20, int(params.get("threshold", 2)), key=f"p_um_t_{idx}"))

    elif t == "restore_photo":
        _upd("remove_scratches", st.checkbox("检测并修复划痕", params.get("remove_scratches", True), key=f"p_rp_rs_{idx}"))
        _upd("denoise", st.checkbox("降噪处理", params.get("denoise", True), key=f"p_rp_d_{idx}"))
        _upd("color_correct", st.checkbox("褪色校正", params.get("color_correct", True), key=f"p_rp_cc_{idx}"))
        _upd("enhance_contrast", st.checkbox("对比度增强", params.get("enhance_contrast", True), key=f"p_rp_ec_{idx}"))
        ...
        if style_items:
            ...
            sel = st.selectbox("选择风格", labels, index=keys.index(cur) if cur in keys else 0, key=f"p_st_{idx}")
            _upd("style", keys[labels.index(sel)])
        ...
        sel = st.selectbox("目标格式", fmt_labels, index=fmts.index(cur) if cur in fmts else 0, key=f"p_fc_f_{idx}")
        _upd("target_format", fmts[fmt_labels.index(sel)])
        _upd("quality", st.slider("压缩质量", 1, 100, int(params.get("quality", 95)), key=f"p_fc_q_{idx}"))
        _upd("optimize", st.checkbox("优化编码", params.get("optimize", True), key=f"p_fc_o_{idx}"))
        _upd("keep_exif", st.checkbox("保留EXIF元数据", params.get("keep_exif", True), key=f"p_fc_e_{idx}"))
